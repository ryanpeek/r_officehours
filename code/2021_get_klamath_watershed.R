...
hucs <- hucs %>% 
  mutate(HUC_6 = substr(HUC_8, 1, 6)) %>% 
  relocate(HUC_6, .after=OBJECTID)

hucKlam <- hucs %>% 
  dplyr::filter(HUC_6 == "180102")

h6_bound <- hucKlam %>% group_by(HUC_6) %>% 
...
mapview(h6_bound) + mapview(klam_out, col.regions="yellow")

# save out
write_rds(hucKlam, file = "data_output/klam_h12.rds")
...
flowline <- navigate_nldi(list(featureSource = "comid", 
                          ...
                          distance_km = 1000)
# write out:
write_rds(x = flowline, file = "data_output/klam_flowline_mainstem.rds")
save(flowline, file = "data_output/klam_flowline_mainstem.rda")

flowlineUT <- navigate_nldi(list(featureSource = "comid", 
                               featureID = start_comid), 
                          mode = "upstreamTributaries", 
                          #mode="UM",
                          distance_km = 1000)
# write out:
save(flowlineUT, file = "data_output/klam_flowline_UT.rda")

mapview(flowline, color="darkblue", lwd=1, show.legend=FALSE) + mapview(h6_bound) +
  mapview(flowlineUT, color="steelblue", lwd=0.5, show.legend=FALSE)

subset <- subset_nhdplus(comids = flowlineUT$nhdplus_comid,
                         output_file = "data_output/klam_watershed_full.gpkg",
...
st_write(ecoreg_l, "data_output/klam_watershed_full.gpkg", layer = "ecoreg_ca_or")
